...
export const metadata: Metadata = {
  ...
  description:
    "Explore SAIT’s fictional September 2026 event schedule: workshops, hackathons, talks, and community meetups, with dates, venues, and demo registration details.",
};
...
const formats = [
  {
    ...
    category: "Build in a day",
    text: "A focused prototype sprint: begin with a problem, build a small solution, and end with a demo. Designed to make trying an idea feel achievable.",
  },
  {
    title: "SAIT HackNight",
    category: "Go deeper, together",
    text: "An overnight team format for campus tools, with room for iteration, peer feedback, and learning from another team’s approach.",
  },
  {
    title: "SAIT Meet & Connect",
    category: "Find your people",
    text: "An informal community format that introduces teams and gives students a place to share interests before choosing what to get involved in.",
  },
];
      ...
      <details className="mt-3 rounded-[var(--radius-md)] border border-line bg-surface text-foreground">
        ...
        <div className="space-y-3 border-t border-line px-4 py-4 text-sm leading-6 text-muted">
          ...
          <p className="font-medium text-foreground">
            Prototype only: registration is not available. Opening this panel
            does not book a place or submit any information.
          </p>
        </div>
      </details>
    </div>
      ...
      >
        <Container className={`${layout} pb-8 pt-14 sm:pt-16 lg:pt-20`}>
          <MotionReveal>
            ...
            <div className="mt-5 grid gap-6 lg:grid-cols-[1.2fr_0.8fr] lg:items-end lg:gap-16">
              ...
              >
                Show up curious.
                <br />
                <span className="text-accent">Leave with more.</span>
              </h1>
              <p className="max-w-xl text-base leading-7 text-muted sm:text-lg sm:leading-8">
                Find your next workshop, build sprint, or community meetup.
                Check the details, choose what interests you, and make room to
                try something new.
              </p>
            </div>
          </MotionReveal>
          ...
          <p className="mt-3 text-xs leading-6 text-muted">
            Demo schedule · September 2026. Event details and registration
            ...
            <MotionReveal>
              ...
              <article className="mt-6 grid overflow-hidden rounded-[var(--radius-xl)] border border-line shadow-[var(--shadow-sm)] lg:grid-cols-[1.1fr_0.9fr]">
                <div className="bg-surface-ink p-6 text-nav-fg sm:p-9 lg:p-10">
                  <div className="flex flex-wrap items-center gap-3 font-mono text-xs uppercase tracking-[0.12em] text-nav-muted">
                    <span>{eventCategoryLabels[nextEvent.kind]}</span>
                    <span aria-hidden="true">/</span>
                    <span>Nearest upcoming event</span>
                  </div>
                  <h2
                    id="next-heading"
                    className="mt-7 font-display text-3xl font-bold tracking-[-0.04em] sm:text-4xl lg:text-5xl"
                  >
                    {nextEvent.title}
                  </h2>
                  <p className="mt-5 max-w-xl text-base leading-8 text-nav-muted">
                    {nextEvent.summary}
                  </p>
                  <p className="mt-7 border-t border-line pt-5 font-mono text-sm leading-7 text-nav-fg">
                    {eventDates(nextEvent)}
                  </p>
                </div>
                <div className="bg-background p-6 sm:p-9 lg:p-10">
                  <dl className="space-y-5">
                    <div>
                      ...
                      <dd className="mt-2 text-lg font-semibold text-foreground">
                        <time
                          dateTime={`${nextEvent.date}T${nextEvent.startTime}:00+05:30`}
                        >
                          {formatEventTime(nextEvent.startTime)}
                        </time>{" "}
                        – {formatEventTime(nextEvent.endTime)} IST
                      </dd>
                    </div>
                    <div>
                      <dt className="font-mono text-xs uppercase tracking-[0.12em] text-muted">
                      ...
                      <dd className="mt-2 flex items-start gap-2 text-base font-medium text-foreground">
                        <MapPin
                          size={18}
                          aria-hidden="true"
                  ...
                  <div className="mt-6 border-t border-line pt-5">
                    <RegistrationInfo event={nextEvent} />
                  </div>
                </div>
              </article>
            </MotionReveal>
        ...
        <Container className={`${layout} ${sectionSpace}`}>
          <MotionReveal>
            <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
              <div>
                <p className={label}>02 / Coming up</p>
                <h2
                  id="upcoming-heading"
                  className="text-section mt-5 text-foreground"
                >
                  Make a little room in your week.
                </h2>
              </div>
              <p className="font-mono text-xs text-muted">
                September 2026 · After next up
              </p>
            </div>
            <div
              className="mt-6 flex flex-wrap items-center gap-2"
              aria-label="Event types in the demo schedule"
            >
              <span className="mr-2 text-xs text-muted">Event types</span>
              {Object.values(eventCategoryLabels).map((category) => (
            ...
            {upcomingEvents.slice(1).map((event) => (
              ...
              >
                <article
                  ...
                  className="grid gap-5 sm:grid-cols-[80px_1fr] sm:gap-7 lg:grid-cols-[88px_1fr_270px] lg:gap-8"
                >
                  <time
                    dateTime={event.date}
                    aria-label={formatEventDate(event.date)}
                    className="flex w-fit items-baseline gap-2 font-mono text-muted sm:flex-col sm:items-start sm:gap-1"
                  >
                    <span className="font-display text-3xl font-bold tracking-[-0.04em] text-foreground sm:text-4xl">
                      {event.date.slice(8)}
                    </span>
                  ...
                  <div className="min-w-0">
                    <p className="font-mono text-xs uppercase tracking-[0.12em] text-accent-hover">
                      {eventCategoryLabels[event.kind]}
                    </p>
                    <h3
                      id={`title-${event.id}`}
                    ...
                    <dl className="mt-3 space-y-2 text-sm text-foreground">
                      <div className="flex items-start gap-2">
                        ...
                        <dd>
                          ...
                          {formatEventTime(event.endTime)} IST
                          {event.endDate && (
                            <span className="block text-xs leading-6 text-muted">
                              Overnight · ends {formatEventDate(event.endDate)}
                            </span>
                          )}
                        </dd>
                      </div>
      ...
      >
        <Container className={`${layout} ${sectionSpace}`}>
          <MotionReveal>
            ...
            <p className="mt-4 max-w-2xl text-sm leading-7 text-muted">
              Recurring-format concepts for this prototype, not claims about
              SAIT&apos;s event history.
            </p>
            <div className="mt-8 grid gap-7 md:grid-cols-3">
              {formats.map((format) => (
                ...
                >
                  ...
                  <h3 className="mt-4 font-display text-xl font-semibold tracking-[-0.03em] text-foreground">
                    {format.title}
                  </h3>
      ...
      >
        <Container className={`${layout} ${sectionSpace}`}>
          <MotionReveal>
            ...
            <p className="mt-4 max-w-2xl text-sm leading-7 text-muted">
              Moments from across the community, collected as an illustrative
              event archive.
            </p>
          </MotionReveal>

          <div className="event-gallery mt-10" aria-label="Past event archive">
            <div className="event-gallery-track">
              {/* First copy */}
              <div className="event-gallery-group">
                {pastEvents.map((event, index) => (
                  <article
                    key={`primary-${event.id}`}
                    className="event-gallery-card group"
                  >
                    <div className="event-gallery-visual">
                      <div className="event-gallery-number" aria-hidden="true">
                      ...
                      <div className="relative z-10">
                        ...
                        <time
                          dateTime={event.date}
                          className="mt-3 block font-mono text-xs text-nav-muted"
                        >
                          {formatEventDate(event.date)}
                        </time>
                      </div>

                      <h3 className="relative z-10 mt-auto max-w-sm font-display text-2xl font-bold tracking-[-0.04em] text-nav-fg sm:text-3xl">
                        {event.title}
                      </h3>
                    </div>
                    ...
                    <div className="flex flex-1 flex-col p-6">
                      ...
                      <div className="mt-auto pt-5">
                        <div className="flex items-center justify-between border-t border-line pt-4">
                          <span className="font-mono text-[10px] uppercase tracking-[0.14em] text-accent-hover">
                            Event recap
                          </span>
              ...
              <div className="event-gallery-group" aria-hidden="true">
                {pastEvents.map((event, index) => (
                  <article
                  ...
                  >
                    <div className="event-gallery-visual">
                      <div className="event-gallery-number">
                        {String(index + 1).padStart(2, "0")}
                      </div>
                      ...
                      <div className="relative z-10">
                        ...
                        <span className="mt-3 block font-mono text-xs text-nav-muted">
                          {formatEventDate(event.date)}
                        </span>
                      </div>
          ...
          <p className="mt-5 font-mono text-[10px] uppercase tracking-[0.14em] text-muted">
            Illustrative event archive · demo content
          </p>
        </Container>
      </section>
        ...
        <Container className={`${layout} ${sectionSpace}`}>
          <MotionReveal>
            <div className="grid gap-8 rounded-[var(--radius-xl)] border border-line bg-surface-ink p-6 text-nav-fg sm:p-10 lg:grid-cols-[1.1fr_0.9fr] lg:items-center lg:gap-14 lg:p-12">
              <div>
                <p className={label}>Be part of what&apos;s next</p>
                <h2
                  id="join-heading"
                ...
                <p className="mt-5 max-w-xl text-base leading-7 text-nav-muted">
                  Attend, help organise, or suggest a session you&apos;d love to
                  see. A useful skill, a good question, or a willingness to help
                  is a place to start.
                </p>
              </div>
