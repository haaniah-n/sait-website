...
const archiveYears = Array.from(
  new Set(archive.map((achievement) => achievement.year)),
);
...
const culture = [
  {
    ...
    icon: Code2,
    description:
      "Make an idea useful. Share the experiment and what it taught you.",
  },
  {
    title: "Research",
    icon: FlaskConical,
    description:
      "Ask a better question. Test it carefully and make your findings useful to others.",
  },
  {
    title: "Compete",
    icon: Flag,
    description:
      "Take on a challenge. Grow through practice, pressure, and teamwork.",
  },
  {
    title: "Learn",
    icon: BookOpen,
    description:
      "Go beyond what you know. Help someone else take their next step.",
  },
  {
    title: "Contribute",
    icon: HeartHandshake,
    description:
      "Leave something better. Good documentation and a helping hand count too.",
  },
];
          ...
          <MotionReveal>
            ...
            <div className="mt-5 grid gap-6 lg:grid-cols-[1.2fr_0.8fr] lg:items-end lg:gap-16">
              ...
              >
                The effort behind
                <br />
                <span className="text-accent">every milestone.</span>
              </h1>
              <p className="max-w-xl text-base leading-7 text-muted sm:text-lg sm:leading-8">
                Some moments end on a stage. Others start with a question, a
                first build, or a helping hand. Here&apos;s to the effort that
        ...
        <Container className={`${layout} ${sectionSpace}`}>
          <MotionReveal>
            <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
              <div>
                <p className={label}>01 / Achievement spotlight</p>
                <h2
            ...
            {spotlightAchievements.map((achievement, index) => {
              ...
              return (
                <MotionReveal
                  key={achievement.id}
                  delay={index * 0.05}
                  className={primary ? "h-full lg:row-span-2" : "h-full"}
                >
                  <article
                    aria-labelledby={`spotlight-${achievement.id}`}
                    className={`flex h-full flex-col rounded-[var(--radius-xl)] border p-6 sm:p-8 ${primary ? "border-line bg-surface-ink text-nav-fg shadow-[var(--shadow-md)] lg:p-10" : "border-line bg-background text-foreground"}`}
                  >
                    <div
                      className={`flex flex-wrap items-center justify-between gap-3 font-mono text-xs ${primary ? "text-nav-muted" : "text-muted"}`}
                    >
                    ...
                    <h3
                      id={`spotlight-${achievement.id}`}
                      className={`mt-7 font-display font-bold leading-tight tracking-[-0.035em] ${primary ? "max-w-md text-3xl sm:text-4xl lg:mt-12 lg:text-5xl" : "text-2xl sm:text-3xl"}`}
                    >
                      {achievement.title}
                    </h3>
                    <p
                    ...
                    <dl
                      className={`mt-auto grid gap-5 pt-7 ${primary ? "sm:pt-12" : "sm:grid-cols-2"}`}
                    >
                      <div
                        className={`border-t pt-5 ${primary ? "border-line" : "border-line"}`}
                      >
                        <dt
                        ...
                        <dd
                          className={`mt-2 font-display font-bold tracking-[-0.04em] ${primary ? "text-4xl text-nav-fg sm:text-5xl" : "text-2xl text-accent-hover"}`}
                        >
                          {achievement.result}
                        </dd>
                      </div>
        ...
        <Container className={`${layout} ${sectionSpace}`}>
          <MotionReveal>
            <div className="grid gap-5 lg:grid-cols-2 lg:items-end lg:gap-16">
              <div>
                <p className={label}>02 / Achievement archive</p>
                <h2
                  id="archive-heading"
                  className="text-section mt-5 text-foreground"
                >
                  Every step adds to the story.
                </h2>
              </div>
          ...
          <div className="mt-10 space-y-10">
            {archiveYears.map((year) => (
              <section
                ...
                className="grid gap-5 border-t border-line pt-6 md:grid-cols-[0.3fr_1fr] md:gap-10"
              >
                <MotionReveal>
                  <h3
                    id={`archive-${year}`}
                    className="font-display text-4xl font-bold tracking-[-0.04em] text-foreground"
                  >
                    {year}
                  </h3>
                        ...
                        <MotionReveal delay={index * 0.03}>
                          <article>
                            <div className="flex flex-wrap items-center gap-x-4 gap-y-2 font-mono text-xs">
                              <p className="uppercase tracking-[0.1em] text-accent-hover">
                                {achievement.categoryLabel}
                              </p>
                              <time
                            ...
                            <h4 className="mt-3 text-lg font-semibold tracking-[-0.025em] text-foreground sm:text-xl">
                              {achievement.title}
                            </h4>
                            ...
                            <dl className="mt-5 flex flex-col gap-x-8 gap-y-3 sm:flex-row sm:flex-wrap">
                              <div>
                                ...
                                <dd className="inline-flex rounded-full bg-accent-soft px-3 py-1 text-xs font-medium leading-5 text-accent-fg">
                                  {achievement.result}
                                </dd>
                              </div>
        ...
        <Container className={`${layout} ${sectionSpace}`}>
          <MotionReveal>
            ...
            <div className="mt-5 grid gap-5 lg:grid-cols-2 lg:gap-16">
              <h2 id="culture-heading" className="text-section">
                Progress has more than one shape.
              </h2>
              <p className="max-w-xl text-sm leading-7 text-nav-muted">
                A podium is one kind of milestone. Sharing a discovery, teaching
                a friend, or making a tool more accessible deserves a place in
                the story too.
              </p>
            </div>
            ...
            <div className="grid gap-8 rounded-[var(--radius-xl)] border border-line bg-background p-6 sm:p-10 lg:grid-cols-[1.1fr_0.9fr] lg:items-center lg:gap-14 lg:p-12">
              <div>
                ...
                >
                  Make something
                  <br />
                  <span className="text-accent">worth sharing.</span>
                </h2>
                <p className="mt-5 max-w-xl text-base leading-7 text-muted">
                  Take on a challenge, explore a research question, or help a
                  team move forward. Your next step doesn&apos;t have to be big
                  to matter.
                </p>
              </div>
              <div className="border-t border-line pt-7 lg:border-l lg:border-t-0 lg:pl-8 lg:pt-0">
                ...
                <p className="mt-3 text-sm leading-7 text-muted">
                  Discover what students are building, or find an event where
                  you can learn, collaborate, and put your skills to the test.
                </p>
